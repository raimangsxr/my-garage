...
Tras resolverlo, el backend quedó arrancando correctamente pero el cluster siguió marcando probes 404 porque en ejecución estaba consultando `/healthz` mientras la aplicación solo exponía `/health`.
...
- Conseguir que los probes operativos del backend respondan tanto en `/health` como en `/healthz`.
...
- Probes de plataforma o manifests desalineados: el backend debe tolerar la variante operativa `/healthz` además de `/health`.
...
- RF-5: El backend debe responder al healthcheck operativo en `/health` y `/healthz` con la misma semántica.
...
- CA-5: Dado un probe HTTP a `/healthz`, cuando el backend está arrancado, entonces responde 200 con el mismo payload que `/health`.
...
- Backend: validar `GET /health` y `GET /healthz`
- Manual/UI: no aplica

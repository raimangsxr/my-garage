...
export class VehicleDocsAiComponent implements OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    this.stopVoiceListening({
      finalize: false,
      resetTo: this.canUseVoiceInput ? 'idle' : 'unsupported',
    });
    ...
    this.ragService
      ...
      .subscribe({
        next: (documents) => {
          ...
          if (
            this.selectedTabIndex === VehicleDocsAiComponent.ASK_TAB_INDEX &&
            this.readyDocumentsCount > 0 &&
            !this.voiceCancelledByUser
          ) {
  ...
  onFilesSelected(event: Event): void {
    ...
    files.forEach((file) => {
      ...
      this.ragService.uploadDocument(this.vehicleId, file, this.selectedDocumentType).subscribe({
  ...
  reindexDocument(document: VehicleDocument): void {
    this.ragService.reindexDocument(document.id).subscribe({
      next: () => {
        this.showSnackBar(
          'Document queued for reindexing. Watch the status pill for progress.',
          'info',
        );
        this.loadDocuments({ silent: true, merge: true });
      },
  ...
  toggleDocumentInChat(document: VehicleDocument): void {
    this.ragService
      .updateDocument(document.id, { included_in_rag: !document.included_in_rag })
      .subscribe({
        next: () => {
          this.showSnackBar(
  ...
  get indexingDocumentsCount(): number {
    return this.documents.filter(
      (document) => document.status === 'uploaded' || document.status === 'indexing',
    ).length;
  }
  ...
  get voicePreviewText(): string {
    return this.voicePreviewTranscriptWithoutWakePhrase();
  }
    ...
    for (const document of documents) {
      ...
      if (previousStatus && previousStatus !== document.status) {
        if (document.status === 'indexing') {
          ...
          this.showSnackBar(
            `${document.title || document.file_name} indexed successfully.`,
            'success',
          );
  ...
  private ensureVoiceArmed(): void {
    ...
    this.setVoiceState(
      'armed',
      `Listening in Ask. Say "${this.voiceWakePhrase}" to start your question.`,
    );
    ...
    try {
      ...
      this.setVoiceState(
        'error',
        'We could not start voice listening in this browser.',
        'Retrying automatically. You can keep typing if needed.',
      );
  ...
  private handleVoiceResult(event: BrowserSpeechRecognitionEventLike): void {
    for (let index = event.resultIndex; index < event.results.length; index += 1) {
      ...
      if (this.voiceState === 'armed') {
        if (result.isFinal) {
          this.activationFinalTranscript = normalizeTranscript(
            `${this.activationFinalTranscript} ${transcript}`,
          );
        ...
        const combinedActivationTranscript = normalizeTranscript(
          `${this.activationFinalTranscript} ${this.activationInterimTranscript}`,
        );
        const activation = extractQueryAfterWakePhrase(
          combinedActivationTranscript,
          this.voiceWakePhrase,
        );

        if (activation.detected) {
          this.setVoiceState(
            'listening',
            'Listening to your question. Stop speaking or press the button when finished.',
          );
          this.voiceFinalTranscript = stripWakePhraseOccurrences(
            activation.queryText,
            this.voiceWakePhrase,
          );
      ...
      const sanitizedTranscript = stripWakePhraseOccurrences(transcript, this.voiceWakePhrase);
      if (!sanitizedTranscript) {
        continue;
      }

      if (result.isFinal) {
        this.voiceFinalTranscript = mergeRecognitionTranscript(
          this.voiceFinalTranscript,
          sanitizedTranscript,
        );
        this.voiceInterimTranscript = '';
      } else {
        this.voiceInterimTranscript = sanitizedTranscript;
      }
      this.scheduleVoiceSilenceTimeout();
    }
  }
  ...
  private handleVoiceError(event: Event & { error?: string }): void {
    ...
    if (event.error === 'not-allowed' || event.error === 'service-not-allowed') {
      ...
      this.setVoiceState(
        ...
        'Allow microphone access in the browser or keep typing your question.',
      );
      return;
    }
  ...
  private handleVoiceEnd(): void {
    ...
    if (
      this.voiceState === 'armed' &&
      this.shouldRestartVoiceRecognition &&
      !this.manualVoiceStopRequested
    ) {
      try {
    ...
    if (this.manualVoiceStopRequested) {
      ...
      if (this.voiceState !== 'ready') {
        this.setVoiceState(
          this.canUseVoiceInput ? 'idle' : 'unsupported',
  ...
  private finalizeVoiceTranscript(
    emptyMessage = 'We could not build a question from that recording.',
  ): void {
    this.clearVoiceSilenceTimer();
    this.shouldRestartVoiceRecognition = false;
    this.manualVoiceStopRequested = false;
    ...
    this.setVoiceState(
      'ready',
      'Voice transcription ready. Still listening for the next question.',
    );
    if (
      !this.voiceCancelledByUser &&
      this.selectedTabIndex === VehicleDocsAiComponent.ASK_TAB_INDEX
  ...
  private voicePreviewTranscriptWithoutWakePhrase(): string {
    const transcript = normalizeTranscript(
      `${this.voiceFinalTranscript} ${this.voiceInterimTranscript}`,
    );
    if (!transcript) {
      return '';
    }

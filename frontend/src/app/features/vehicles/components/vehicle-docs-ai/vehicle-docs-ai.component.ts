...
export class VehicleDocsAiComponent implements OnInit, OnDestroy {
        ...
        this.ragService.listDocuments(this.vehicleId)
            ...
            .subscribe({
                ...
                error: (error) => {
                    ...
                    this.documentsErrorMessage = error?.error?.detail || 'We could not load the vehicle documents.';
                    if (!options.silent) {
                        this.showSnackBar(this.documentsErrorMessage, 'error');
                    }
                }
            });
    }
    ...
    retryLastQuestion(): void {
        const question = this.lastFailedQuestion.trim();
        if (!question || this.asking) {
            return;
        }
    ...
    private submitQuestion(question: string): void {
        ...
        this.ragService.ask(this.vehicleId, {
            ...
            error: (error) => {
                ...
                this.askErrorMessage = error?.error?.detail || 'We could not get an answer from the vehicle assistant.';
                this.lastFailedQuestion = question;
                this.showSnackBar(this.askErrorMessage, 'error');
            }
        });
    }
    ...
    private handleVoiceError(event: Event & { error?: string }): void {
        ...
        if (event.error === 'no-speech') {
            this.logger.info('Browser speech recognition timed out without speech', event);
            if (this.voiceState === 'armed') {
                this.setVoiceState('armed', `Listening in Ask. Say "${this.voiceWakePhrase}" to start your question.`);
                return;
            }
        ...
        if (event.error === 'not-allowed' || event.error === 'service-not-allowed') {
            this.logger.warn('Browser speech recognition was blocked by microphone permissions', event);
            this.shouldRestartVoiceRecognition = false;
            this.voiceCancelledByUser = true;
        ...
        if (event.error === 'audio-capture') {
            this.logger.warn('Browser speech recognition is unavailable because there is no microphone input', event);
            this.shouldRestartVoiceRecognition = false;
            this.voiceCancelledByUser = true;
            this.setVoiceState('error', 'Microphone input is not available.', 'Use typed questions or reconnect a microphone before retrying voice mode.');
            return;
        }

        if (event.error === 'network') {
            this.logger.warn('Browser speech recognition is temporarily unavailable due to a network issue', event);
            this.shouldRestartVoiceRecognition = false;
            this.voiceCancelledByUser = true;
            this.setVoiceState('error', 'Voice recognition is unavailable right now.', 'Keep typing your question or try voice mode again later.');
            return;
        }

...
Este directorio contiene manifests para desplegar `my-garage` en Kubernetes con Traefik y PostgreSQL externo.

## Estructura

- `namespace.yaml`: namespace `my-garage` con Pod Security `restricted`
...
```bash
docker build -t ghcr.io/<tu-org>/my-garage-backend:<tag> ./backend
docker build -t ghcr.io/<tu-org>/my-garage-frontend:<tag> ./frontend

docker push ghcr.io/<tu-org>/my-garage-backend:<tag>
docker push ghcr.io/<tu-org>/my-garage-frontend:<tag>
```

## Preparar configuración

1. Copia `secret.example.yaml` a un fichero privado (por ejemplo `secret.yaml`) y reemplaza valores.
2. Ajusta en `configmap.yaml` e `ingress.yaml` tu dominio real (por defecto `my-garage.example.com`).
...
kubectl wait --for=condition=complete --timeout=180s job/my-garage-migrate -n my-garage
...
- El backend monta `my-garage-backend-media` en `/app/media`.
